...
このスキルは、作業内容から worktree 作成条件を整理し、再現可能な形で
`scripts/setup_worktree.sh` を実行する。
ブランチ名提案や既定値の判断はエージェントが担当し、決定的な処理はスクリプトへ委譲する。

## 対話方針

- 情報が不足していてリスクがある場合だけ、通常の会話で短く確認する。
- 専用の質問ツール名に依存しない。
- すでに情報が揃っている場合は、追加質問なしで進める。
- 実行前に最低限以下を確定する:
  1. 対応内容（1-2 文）
  2. ブランチ名（未指定なら候補を 2-3 個提案）
...
1. 対応内容、成功条件、制約を整理する。
2. ブランチ名が未指定なら 2-3 個提案し、1 つに絞る。指定済みならそのまま使う。
3. ベース参照を決める。指定がなければ `main` を確認し、存在しない場合は `HEAD` を使う。
4. 初期セットアップは既定で実行する。目的は、作成直後の worktree で親 repo と同じ `task.exe test` / `task.exe verify` / Git hooks の通常フローに入れる状態を作ること。既定では `task.exe install` を実行する。`task.exe` / `task` が見つからない場合は、通常フローに入れないため失敗扱いにする。重い初期化を避けたい場合だけ `--skip-install` を使う。
5. `scripts/setup_worktree.sh` を実行する。Git が `--no-relative-paths` をサポートしている場合、Windows 絶対パスで worktree を作成する。
...
- ユーザーがすでにブランチ名を指定している場合は、不要な再提案をしない。
...
# インストール後に checkout 診断も明示したい場合
bash .codex/skills/worktree-setup/scripts/setup_worktree.sh --branch <branch> --install-cmd "task.exe install && task.exe doctor"
...
親 repo と違う挙動がある場合は、切り分け用に `task.exe doctor` を実行する。
