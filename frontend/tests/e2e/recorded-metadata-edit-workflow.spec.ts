...
test('録画済みメタデータ編集 - 選択フィールドの変更', async ({ page }) => {
  ...
  if (newRuleValue) {
    ...
    const updatedRecordedVideo = await firstRecordedVideo(page);
    const displayedRule = await updatedRecordedVideo
      .getByTestId('recorded-video-rule')
      .textContent();
    ...
    console.log('変更可能なルールオプションが見つかりませんでした（スキップ）');
  }
});
...
test('録画済みメタデータ編集 - 複数フィールドの一括編集', async ({ page }) => {
  ...
  const goldMedalsInput = page.getByLabel('金メダル');
  const silverMedalsInput = page.getByLabel('銀メダル');
...
test('録画済みメタデータ編集 - ダイアログ再オープン時の初期値', async ({ page }) => {
  ...
  // 2回目: 再度編集ダイアログを開く
  const updatedRecordedVideo = await firstRecordedVideo(page);
  const editButtonSecond = updatedRecordedVideo.getByTestId('recorded-video-edit-button');
