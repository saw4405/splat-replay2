<script lang="ts">
  ...
  export async function next(options: { skip?: boolean } = {}): Promise<boolean> {
    ...
    if (!options.skip && !currentStep.completed) {
      await markSubstepCompleted(SetupStep.OBS_SETUP, currentStep.id, true);
    }
  ...
  async function checkOBSInstallation(showError: boolean = false): Promise<void> {
    ...
    try {
      ...
      if (obsInstalled) {
        // インストール済みの場合は必ず状態を更新
        await markSubstepCompleted(SetupStep.OBS_SETUP, setupSteps[0].id, true);
  ...
  async function checkNDIInstallation(showError: boolean = false): Promise<void> {
    ...
    try {
      ...
      if (ndiInstalled) {
        // インストール済みの場合は必ず状態を更新
        await markSubstepCompleted(SetupStep.OBS_SETUP, setupSteps[2].id, true);
  ...
  async function toggleStep(index: number): Promise<void> {
    ...
    // その他のステップは通常通りトグル
    await markSubstepCompleted(SetupStep.OBS_SETUP, step.id, !step.completed);
  }
...
<style>
  ...
  .device-select {
    ...
    border-radius: 8px;
    border: 1px solid rgba(var(--theme-rgb-white), 0.2);
    background-color: rgba(var(--theme-rgb-white), 0.05);
    color: var(--text-primary);
    transition: all 0.2s ease;
    box-sizing: border-box;
    cursor: pointer;
  ...
  .device-select:hover {
    background-color: rgba(var(--theme-rgb-white), 0.1);
    border-color: rgba(var(--theme-rgb-white), 0.3);
  }
  ...
  .device-select:focus {
    outline: none;
    border-color: var(--accent-color);
    box-shadow: 0 0 0 2px rgba(var(--theme-rgb-accent), 0.2);
    background-color: rgba(var(--theme-rgb-white), 0.1);
  }

  .device-select option {
    background-color: var(--theme-color-charcoal);
    color: var(--text-primary);
    padding: 0.5rem;
  }
