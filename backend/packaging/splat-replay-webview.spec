...
cyndilib_binaries = []
cyndilib_path = backend_root / '.venv' / 'Lib' / 'site-packages' / 'cyndilib'
...
exe = EXE(
    ...
    icon=str(backend_root / 'assets' / 'icon.ico'),  # アイコン設定（ICO形式）
)
...
# Note: ビルド後、build:move-to-root タスクでプロジェクトルートの dist/ に移動される
