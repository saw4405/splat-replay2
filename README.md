...
- **Web フロントエンド**: 録画状態の監視やメタデータ編集、イベントログの確認をブラウザ上から行えます。
...
1. `uv run splat-replay serve --reload` を実行し、FastAPI サーバーを起動します。`uv run uvicorn splat_replay.interface.api.main:app --reload` でも同様に起動できます。
2. ブラウザで `http://localhost:8000/health` にアクセスし、`{"status": "ok"}` が表示されることを確認してください。

### フロントエンド (Svelte) の起動

1. `cd frontend` でフロントエンドディレクトリに移動します。
2. `npm install` を実行して依存関係をインストールします。
3. `npm run dev` で開発サーバーを起動し、`http://localhost:5173` をブラウザで開きます。
4. ブラウザ上のダッシュボードから録画制御・自動処理・メタデータ編集などの操作を行えます。

### 開発サーバーをまとめて起動する

`uv run splat-replay dev` を実行すると FastAPI と Svelte の開発サーバーを同時に起動できます。初回実行時は `frontend` ディレクトリで `npm install` が自動実行されます。停止する場合は `Ctrl + C` を押してください。
